# Отчет по выполнению практических работ №4-7
...
Целью практических работ №4-7 являлось последовательное изучение фонового выполнения задач, аппаратных возможностей мобильных устройств, способов хранения данных, сетевого взаимодействия и авторизации пользователей в ОС Android. В рамках работ были рассмотрены потоки, механизмы безопасного обновления интерфейса, датчики устройства, камера, аудиозапись, `SharedPreferences`, файловое хранилище, Firebase Authentication, HTTP-запросы, JSON и архитектура приложения `MireaProject` с несколькими функциональными фрагментами.
...
В результате выполнения практических работ №4-7 были освоены важные разделы Android-разработки: многопоточность, взаимодействие с аппаратными возможностями устройства, хранение данных, авторизация пользователей и получение данных из сети. В проекте `MireaProject` реализована навигация через боковое меню, экраны работы с датчиками, камерой, диктофоном, профилем пользователя, зашифрованными файлами, Firebase-входом и сетевым ресурсом. Полученные навыки позволяют создавать приложения, которые не блокируют интерфейс, используют возможности устройства, сохраняют пользовательские данные между запусками и взаимодействуют с удаленными сервисами.
