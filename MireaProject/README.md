...
В ходе выполнения практической работы №4 были изучены базовые принципы многопоточности в Android. Были рассмотрены различия между процессом и потоком, способы создания фоновых задач, а также методы передачи результата в главный поток приложения. На практике были освоены `Thread`, `Runnable`, `Handler`, `Looper`, `MessageQueue` и `Executor`. Полученные знания необходимы для выполнения длительных операций без зависания интерфейса.
...
В ходе выполнения практической работы №5 были изучены аппаратные возможности Android. Был реализован экран датчиков с вычислением направления на север, экран фотозаметок с запуском системной камеры через `FileProvider`, а также диктофон на базе `MediaRecorder` и `MediaPlayer`. Дополнительно был получен практический опыт обработки runtime-разрешений для камеры и микрофона.
...
В ходе выполнения практической работы №6 были изучены основные механизмы хранения данных в Android. Был реализован фрагмент профиля с сохранением параметров пользователя в `SharedPreferences`, а также фрагмент работы с файлами, где записи создаются через `FloatingActionButton`, шифруются и сохраняются в каталоге `Documents` приложения. Дополнительно были рассмотрены внутреннее и внешнее хранилища, защищенные настройки, SQLite и библиотека `Room`.
